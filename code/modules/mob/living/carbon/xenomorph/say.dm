/mob/living/carbon/xenomorph/say(message)
	...
	if(prefix == "*")
		if(!findtext(message, "*", 2)) //Second asterisk means it is markup for *bold*, not an *emote.
			return emote(lowertext(copytext_char(message, 2)), intentional = TRUE)

	var/hivemind_speak = FALSE

	if(prefix == ";")
		message = capitalize(trim_left(copytext_char(message, 2)))
		hivemind_speak = TRUE
	else if(prefix == "." || prefix == "#" || prefix == ":" || prefix == ",")
		message = capitalize(trim_left(copytext_char(message, 3)))
		hivemind_speak = TRUE
