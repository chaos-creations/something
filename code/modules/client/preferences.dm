				...
				if("xeno_prefix")
					...
					if(length_char(new_xeno_prefix)==0)
						...
						for(var/i=1, i<=length_char(new_xeno_prefix), i++)
							var/ascii_char = text2ascii_char(new_xeno_prefix,i)
							if(!((ascii_char >= 65 && ascii_char <= 90) || (ascii_char >= 1040 && ascii_char <= 1071 && ascii_char != 1025)))
				...
				if("xeno_postfix")
					...
					if(length(new_xeno_postfix)>2)
						...
						for(var/i=1, i<=length_char(new_xeno_postfix), i++)
							...
							switch(ascii_char)
								// A  .. Z
								if(65 to 90, 1040 to 1071, 1025) //Uppercase Letters will work on first char
