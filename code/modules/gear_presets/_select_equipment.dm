...
/datum/equipment_preset/proc/load_preset(mob/living/carbon/human/new_human, randomise = FALSE, count_participant = FALSE, client/mob_client, show_job_gear = TRUE, late_join)
	...
	if(origin_override)
		...
		new_human.accent_prefix = GLOB.origins[origin_override].tag_icon
...
/datum/equipment_preset/proc/add_rebel_ua_suit_elite(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/suitpath = pick(
		/obj/item/clothing/suit/marine,
...
/datum/equipment_preset/proc/add_rebel_twe_suit_elite(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/suitpath = pick(
		/obj/item/clothing/suit/marine/veteran/royal_marine,
...
/datum/equipment_preset/proc/add_rebel_ua_helmet_elite(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/helmetpath = pick(
		/obj/item/clothing/head/helmet/marine,
...
/datum/equipment_preset/proc/add_rebel_twe_helmet_elite(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/helmetpath = pick(
		/obj/item/clothing/head/helmet/marine/veteran/royal_marine,
...
GLOBAL_LIST_INIT(rebel_twe_rifles, list(
	...
	/obj/item/weapon/gun/rifle/rmc_f90 = /obj/item/ammo_magazine/rifle/rmc_f90,
	/obj/item/weapon/gun/rifle/rmc_f90/a_grip = /obj/item/ammo_magazine/rifle/rmc_f90,
	))
...
/datum/equipment_preset/proc/add_forecon_surv_uniform(mob/living/carbon/human/new_human)
...
/datum/equipment_preset/proc/add_forecon_uniform(mob/living/carbon/human/new_human)
	var/obj/item/clothing/under/marine/standard/uniform = new() //Survivor presets are the 'seen some shit' sort, these guys are fresher
	var/random_uniform = rand(1,3)
	switch(random_uniform)
		if(1)
			uniform.roll_suit_jacket(new_human)
...
/datum/equipment_preset/proc/add_rebel_upp_suit_elite(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/suitpath = pick(
		/obj/item/clothing/suit/marine/faction/UPP/CANC,
...
/datum/equipment_preset/proc/add_rebel_upp_helmet_elite(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/helmetpath = pick(
		/obj/item/clothing/head/helmet/marine/veteran/UPP/old,
...
/datum/equipment_preset/proc/merc_backpack(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/random_satchel = pick(
...
/datum/equipment_preset/proc/merc_helmet_ua(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/helmetpath = pick(
		/obj/item/clothing/head/helmet/marine,
...
/datum/equipment_preset/proc/merc_helmet_upp(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/helmetpath = pick(
...
/datum/equipment_preset/proc/merc_helmet_other(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/helmetpath = pick(
		/obj/item/clothing/head/helmet/marine/veteran/royal_marine,
...
/datum/equipment_preset/proc/merc_eyes(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/eyespath = pick(
		/obj/item/clothing/glasses/mgoggles,
...
/datum/equipment_preset/proc/merc_facewrap(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/random_facewrap = pick(
		/obj/item/clothing/mask/rebreather/scarf/tacticalmask,
...
/datum/equipment_preset/proc/merc_uniform_ua(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/uniformpath = pick(
		//USCM
...
/datum/equipment_preset/proc/merc_uniform_upp(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/uniformpath = pick(
		//upp
...
/datum/equipment_preset/proc/merc_uniform_other(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/uniformpath = pick(
		//other
...
/datum/equipment_preset/proc/merc_accessory_cosmetic(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/accessorypath = pick(
		/obj/item/clothing/accessory/armband,
...
/datum/equipment_preset/proc/merc_accessory_useful(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/accessorypath = pick(
...
/datum/equipment_preset/proc/merc_accessory_helmet(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/accessorypath = pick(
...
/datum/equipment_preset/proc/merc_suit_ua(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/suitpath = pick(
		/obj/item/clothing/suit/marine,
...
/datum/equipment_preset/proc/merc_suit_upp(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/suitpath = pick(
		/obj/item/clothing/suit/marine/faction/UPP/CANC,
...
/datum/equipment_preset/proc/merc_suit_other(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/suitpath = pick(
		/obj/item/clothing/suit/marine/veteran/royal_marine,
...
/datum/equipment_preset/proc/merc_shoes_ua(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/shoespath = pick(
		/obj/item/clothing/shoes/marine/knife,
...
/datum/equipment_preset/proc/merc_shoes_upp(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/shoespath = pick(
...
/datum/equipment_preset/proc/merc_shoes_other(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/shoespath = pick(
		...
		/obj/item/clothing/shoes/marine/civilian/jungle/knife,
		/obj/item/clothing/shoes/marine/civilian/knife,
...
/datum/equipment_preset/proc/merc_gloves(mob/living/carbon/human/new_human)
	if(!istype(new_human))
		return
	var/glovespath = pick(
		/obj/item/clothing/gloves/marine/brown,
