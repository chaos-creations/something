...
/datum/fluff_email/rover/mission1
	title = "Эвакуация Предпринемателя"
	entry_text = {"<html>
					По личным каналам связи поступил сигнал бедствия с шахтерской колонии на пограничном мире LV-976, «Сорокинский форпост»
					Колония раньше была под властью Союза, но они её давно покинули и в основном теперь там правят 'Дикие', но видимо наш заказчик решился поскупиться на охрану.
					<br>
					Ваша задача - определить местоположение заказчика с помощью оборудования в колонии, найти его и эвакуировать либо с помощью тросса, либо с ближайшего ЛЗ.
					<br>
					Награда за выполненную работу - 40 RP на весь отряд.
				</html>
				"}

/datum/fluff_email/rover/mission2
	title = "Подопытные крысы"
	entry_text = {"<html>
					Нашему дорогому торговцу с Аскелапия срочно понадобились испытуемые для его новых биологических экспериментов.
					Необходимо найти и захватить как минимум двух человек, живыми. Мёртвые не подходят, чтобы получить необходимые инструменты для захвата обратитесь к торговцу в Аскелапии.
					<br>
					Награда за выполненную работу - 32 RP, 1 LC(Scholar).
				</html>
				"}
